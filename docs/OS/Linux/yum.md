...
适用于 CentOS、Fedora 和 RHEL 操作系统的包管理工具 yum（yellow dog updater, modified），使用 rpm（redhat package manager）格式安装包。
...
```fish
# 检查可用升级
user@host *> yum check-update

# 升级
user@host *> sudo yum upgrade

# 搜索
user@host *> yum search <query>

# 显示包信息
user@host *> yum info <package>

# 安装包
user@host *> sudo yum install <package>

# 卸载包
user@host *> sudo yum remove <package>
```
